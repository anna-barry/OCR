  ...
  for (int j = 0; j < m.width ; j++) {//vertical search in the matrix
    ...
    while (i < m.height) {
      ...
      if ((int)m.matrix[i*m.width+j]!=1){
        ...
        k=i;
        nbzeros=0;
        while ((int)m.matrix[i*m.width+k]!=1&&k< m.height) {//counting the numbers of 0 followed between two 1
...
int main()
{
    ...
    //Matrix m = rlsa(derhistoMat,30,30);

    Matrix m = new_alea_Matrix(20, 20);
