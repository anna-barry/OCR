...
int main()
{
    ...
    screen_surface = display_image(img);

    update_surface(screen_surface,img);
    ...
    imgf = SDL_CreateRGBSurface(0, M.width, M.height, 32, 0, 0, 0, 0);
	...
	if (M.matrix[i*M.width+j]==1)
	  {
	    ...
	    pixel = SDL_MapRGB(imgf->format, 0, 0, 0);
	  }
	else{

	  pixel = SDL_MapRGB(imgf->format, 255, 255, 255);
	}

	put_pixel(imgf, i, j, pixel);
      }
    }

    SDL_SaveBMP(imgf, "new");
